...
class ApiModel
{
    ...
    public function getColumns($table, $enrich = false, $required = false)
    {
        if ($required != false) {
            ...
            $this->db->query("SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = {$table} AND is_nullable = 'YES'");
        }#No lo se veo normailito xd
        $this->result = $this->db->registrosrow();
        $this->table_colums = array();
#El valor de value no tiene que ser columname?
        foreach ($this->result as $key => $value) {
            if ($this->getPrimaryKey($table) != $value['column_name']) {
                if ($enrich != false) {
                    array_push($this->table_colums, ':' . $value['column_name']);
                } else {
                    array_push($this->table_colums, $value['column_name']);
                }
            }
    ...
    {
        $this->db->query("SELECT * FROM pg_catalog.pg_tables WHERE schemaname != 'pg_catalog' AND schemaname != 'information_schema' ");
        $this->result = (array)  $this->db->registrosrow();

        $this->table = array();

        foreach ($this->result as $key => $value) {
            array_push($this->table, $value['tablename']);
